...
def test_guest_cant_see_success_message_after_adding_product_to_basket(browser): # негативные проверки с сообщением об успехе
...
def test_guest_cant_see_success_message(browser):    # негативные проверки с сообщением об успехе
    link = "http://selenium1py.pythonanywhere.com/ru/catalogue/coders-at-work_207/"
    page = ProductPage(browser, link)  # инициализируем Page Object, передаем в конструктор экземпляр драйвера и url адрес
    page.open()  # открываем страницу товара
...
@pytest.mark.xfail  # тест упадет, помечаем как ожидаемо падающий
def test_message_disappeared_after_adding_product_to_basket(browser):    # негативные проверки с сообщением об успехе
...
def test_guest_should_see_login_link_on_product_page(browser):  # проверяем, что видим ссылку для регистрации
    link = "http://selenium1py.pythonanywhere.com/en-gb/catalogue/the-city-and-the-stars_95/"
    page = ProductPage(browser, link)
    page.open()
    page.should_be_login_link()

def test_guest_can_go_to_login_page_from_product_page(browser): # проверяем, что можем перейти на страницу регистрации
    link = "http://selenium1py.pythonanywhere.com/en-gb/catalogue/the-city-and-the-stars_95/"
    page = ProductPage(browser, link)
    page.open()
